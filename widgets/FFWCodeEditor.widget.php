...
/**
 * Classe do widget GtkCodeEditor
 * 
 * @name Fabula::GtkCodeEditor()
 * @see GtkSourceview
 * @example GtkCodeEditor.php
 * @package Fabula
 */
class FFWCodeEditor extends GtkSourceView {	
	/**
	...
	public function __construct($window) {
		...
		$this->__widgets['autocomplete'] = new FFWCodeEditorAutocomplete(NULL);
	}
	...
	public function __inserted_text($widget, $iter, $arg1, $arg2) {
		...
		if(strlen($this->__last_token) >= 2) {
			...
			// Verifica se está dentro de um scrolledwindow
			$scrolledwindow = $this->get_parent();
			if(strtolower(get_class($scrolledwindow)) != "gtkscrolledwindow") {
				throw new Exception("O widget GtkCodeEditor não parece estar dentro de um GtkScrolledWindow");
			}
			
			// Busca a posição do scroll vertival
			$vscroll = $scrolledwindow->get_vscrollbar();
			$hscroll = $scrolledwindow->get_hscrollbar();
			
			// Calcula a posição do dialog
			$x = $cursor_location->x + $window_location[0] - ((int)$hscroll->get_value());
			$y = $cursor_location->y + $window_location[1] + 15 - ((int)$vscroll->get_value());
...
*/
/*
			$window = $this->get_window(GTK::TEXT_WINDOW_WIDGET);
			$window->focus();
			var_dump(get_class($window));
*/
			$this->window->grab_focus();
		}
	}
